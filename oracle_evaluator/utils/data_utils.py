...
class SegmData:
    ...
    def generate_data_paths(self) -> (tuple[str], tuple[str]):
        ...
        if "bbox" in self.dataset_dp:
            imgs_paths = []
            masks_paths = []
            for dirs in os.listdir(self.dataset_dp): # video names
                            if ".DS_Store" not in dirs:
                                for frames in os.listdir(os.path.join(self.dataset_dp, dirs)): # frames
                                    if ".DS_Store" not in frames and "video.json" not in frames:
                                        for sub_frames in os.listdir(os.path.join(self.dataset_dp, dirs, frames)): # sub frames
                                            if ".DS_Store" not in sub_frames:
                                                imgs_paths.append(os.path.join(self.dataset_dp, dirs, frames, sub_frames, "crop.png").replace("\\",'/'))
                # break
            imgs_paths = sorted(imgs_paths, reverse = True)
            instance_pair_paths = {}
            for img_path in imgs_paths:
                assert os.path.exists(img_path), 'E: Image path does not exist.'
                assert os.path.exists(img_path.replace("crop.png", "mask.png")), 'E: Mask path does not exist.'
                instance_pair_paths[img_path] = img_path.replace("crop.png", "mask.png")
            imgs_paths = []
        elif "smoke" in self.dataset_dp:
            imgs_paths = []
            masks_paths = []
            imgs_folders = os.path.join(self.dataset_dp, 'frames')
            for dirs in os.listdir(imgs_folders): # video names
                # if dirs != "9J-4qvCueZw-2":
                #     continue
                for frames in os.listdir(os.path.join(imgs_folders, dirs)): # frames
                    imgs_paths.append(os.path.join(imgs_folders, dirs, frames).replace("\\",'/'))
                # break
            imgs_paths = sorted(imgs_paths, reverse = True)
            instance_pair_paths = {}
            for img_path in imgs_paths:
                assert os.path.exists(img_path), 'E: Image path does not exist.'
                assert os.path.exists(img_path.replace("frames", "masks").replace("jpg", "png")), 'E: Mask path does not exist.'
                instance_pair_paths[img_path] = img_path.replace("frames", "masks").replace("jpg", "png")
            imgs_paths = []
        elif "S-Smoke" in self.dataset_dp:
            imgs_paths = glob\
            (
                pathname = os.path.join(self.dataset_dp, 'images', '*.jpg').replace("\\",'/'),
                recursive = True
            )

            masks_paths = glob\
            (
                pathname = os.path.join(self.dataset_dp, 'seg_labels', '*.png').replace("\\",'/'),
                recursive = True
            )

            imgs_paths = [img_p.replace("\\",'/') for img_p in imgs_paths]
            masks_paths = [img_p.replace("\\",'/') for img_p in masks_paths]
    ...
    def get_one_segm_instance_ssmoke_dp(self, img_fp: str = None, mask_fp: str = None):
        ...
        # if mask_ssmoke.max() > 1:
        #     mask_ssmoke = mask_ssmoke // 255
        mask_ssmoke[mask_ssmoke > 0] = 1
